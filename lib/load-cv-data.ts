...
export async function getCVLastUpdated(
...
export async function getCVData(
	...
	if (locale !== appConfig.cvData.fallbackLocale) {
		console.warn(
			`Falling back to default locale '${appConfig.cvData.fallbackLocale}'`,
		);
		const fallbackOverride = await tryGetLocalizedTomlOverride(
			appConfig.cvData.fallbackLocale,
		);
		if (fallbackOverride) {
...
async function getCVDataFromLocalizedToml(
	locale: Locale,
): Promise<DeepPartial<CVData>> {
	const fileContents = await fs.readFile(
		getLocalizedTomlFilePath(locale),
		"utf8",
	);
	return parseToml(fileContents) as DeepPartial<CVData>;
}

async function tryGetLocalizedTomlOverride(
	locale: Locale,
): Promise<DeepPartial<CVData> | null> {
	...
	try {
		...
		console.error(
			`Error loading localized TOML override for locale ${locale}:`,
			error,
		);
		return null;
	}
...
function getLocalizedTomlFilePath(locale: Locale): string {
	const fileName = appConfig.cvData.localizedTomlTemplate.replace(
		"{locale}",
		locale,
	);
	...
	return {
		...
		projects: projectEntries.flatMap((item) => {
			...
			const previewImages = (item.preview_images ?? [])
				...
				.filter((previewImage): previewImage is ProjectPreviewImage =>
...
function mapProjectUrl(
	value: string | { label?: string; url?: string; icon?: string },
): ProjectUrl | null {
...
function getSampleData(): CVData {
	return {
		hero: {
			name: "Sample Name",
			avatar: "/images/placeholders/placeholder.svg?height=128&width=128",
		...
		experience: [
			{
				...
				highlights: ["Sample achievement 1", "Sample achievement 2"],
			},
		],
		sectionConfig: {
			education: {
