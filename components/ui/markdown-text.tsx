...
function getComponents(inline: boolean, showLinkIcon: boolean): Components {
	return {
		...
		ul: ({ children }) => (
			<ul className={inline ? "inline" : "list-disc pl-5"}>{children}</ul>
		),
		ol: ({ children }) => (
			<ol className={inline ? "inline" : "list-decimal pl-5"}>{children}</ol>
		),
		li: ({ children }) =>
			inline ? <span>{children}</span> : <li>{children}</li>,
	...
	return (
		<Wrapper className={cn(blockSpacingClass, className)}>
			<ReactMarkdown
				...
				components={getComponents(inline, showLinkIcon)}
			>
				{content}
			</ReactMarkdown>
		</Wrapper>
