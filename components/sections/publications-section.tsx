  ...
  return (
    <section className="paper-section">
      <h2 className="paper-section-title">
        <Icon icon="mingcute:book-6-fill" className="mr-3 inline-block size-[1em] align-[-0.12em] text-primary" />
        {t('sections.publications')}
      </h2>

      <div className="space-y-1.5">
        {sortedItems.map((publication, index) => (
          <div key={`${publication.title}-${index}`} className="paper-body text-foreground leading-relaxed">
            <div className="grid grid-cols-[minmax(0,1fr)_auto] items-start gap-x-3 gap-y-0.5 md:grid-cols-[minmax(6ch,auto)_minmax(0,1fr)] md:gap-y-1">
              ...
              <div className="order-1 min-w-0 space-y-0.5 md:order-2 md:col-start-2">
                ...
                {publication.authors?.length ? (
                  <p className="text-foreground/80 text-sm">{formatAuthors(publication.authors)}</p>
                ) : null}

                {formatPublicationMeta(publication) ? (
                  <p className="text-foreground/80 text-sm italic">{formatPublicationMeta(publication)}</p>
                ) : null}
