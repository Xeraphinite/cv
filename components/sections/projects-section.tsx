  ...
  return (
    <section className="paper-section">
      <h2 className="paper-section-title">
        <Icon icon="mingcute:light-fill" className="mr-3 inline-block size-[1em] align-[-0.12em] text-primary" />
        {t('sections.selectedProjects')}
      </h2>

      <div className="space-y-3">
        {projects.map((project) => (
          <div key={project.name} className="paper-body text-foreground leading-relaxed">
            <div className="grid grid-cols-[minmax(0,1fr)_auto] items-start gap-x-3 gap-y-2 md:grid-cols-[minmax(5ch,auto)_minmax(0,1fr)]">
              <span className="order-2 col-start-2 row-start-1 justify-self-end whitespace-nowrap text-right font-bold font-sans text-foreground/80 text-sm md:order-1 md:col-start-1 md:justify-self-start md:text-left">{formatToYearMonth(project.year)}</span>
              <div className="order-1 col-start-1 row-start-1 flex min-w-0 flex-wrap items-center gap-2 md:order-2 md:col-start-2">
                <h3 className="font-sans font-semibold text-sm">{project.name}</h3>
                {project.status ? (
                  <Badge variant="secondary" className="rounded-full border border-border/60 bg-muted/80 px-2 py-0 font-medium font-sans text-foreground/85 text-xs hover:bg-secondary/80">
                    {project.status}
                  </Badge>
                ) : null}
              </div>
